...
class TBIvisualizer(QtWidgets.QDialog):
    ...
    def __init__(self, strain_path, anatomy_path, head_path):
        ...
        risk = self.caculate_risk(data_strain)
        print("Risk of TBI:", risk)

        uic.loadUi("tbi_viewer2.ui",self)
        self.opacity_slider.valueChanged.connect(self.update_anatomy_opacity)
        self.strip_button.clicked.connect(self.start_strip_animation)
    ...
    def start_strip_animation(self):
        """Triggered when you click the button in the UI"""
        self.current_head_opacity = 1.0
        self.animation_timer.start(30) # 30ms intervals for smooth motion
